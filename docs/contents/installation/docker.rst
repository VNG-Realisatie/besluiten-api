...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it brc_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
...
environment variables. See ``src/brc/conf/docker.py`` for
all settings.
...
.. code-block:: bash
    ...
    $ docker exec -it brc /app/src/manage.py createsuperuser
